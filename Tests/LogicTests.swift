...
struct LogicTests {
    static func main() {
        ...
        expect(WorkflowRunDisplayFormatter.triggeredByText(for: actorRun) == "exodos-harry-zorn", "workflow formatter exposes triggering user separately")
        expect(!WorkflowRunDisplayFormatter.rowMetadata(for: actorRun, now: now).contains("exodos-harry-zorn"), "row metadata keeps triggering user separate from truncating metadata")
        ...
        expect(WorkflowRunDisplayFormatter.triggeredByText(for: actorlessRun) == nil, "workflow formatter omits empty triggering user")
